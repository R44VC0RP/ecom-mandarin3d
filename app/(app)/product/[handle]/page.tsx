...
import { getProduct, getProductRecommendations } from 'lib/prisma-queries';
...
export default async function ProductPage(props: { params: Promise<{ handle: string }> }) {
  ...
  // Parse selectedOptions from JSON if needed
  const variants = product.variants.map(variant => ({
    ...variant,
    selectedOptions: typeof variant.selectedOptions === 'string' 
      ? JSON.parse(variant.selectedOptions)
  ...
  const productJsonLd = {
    ...
    offers: {
      ...
      priceCurrency: product.priceRange?.maxVariantPrice.currencyCode || 'USD',
        ...
        <div className="flex flex-col rounded-lg border border-neutral-200 bg-white p-8 md:p-12 lg:flex-row lg:gap-8 dark:border-neutral-800 dark:bg-black">
          ...
          <div className="basis-full lg:basis-2/6">
            <Suspense fallback={null}>
              <ProductDescription product={productWithParsedVariants} />
            </Suspense>
          </div>
        </div>
